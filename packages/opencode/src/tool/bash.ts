  ...
  return {
    ...
    async execute(params, ctx) {
      ...
      if (Runtime.mode() === "webcontainer") {
        ...
        try {
          const proc = spawn("jsh", ["-c", params.command], {
            cwd,
            env: {
              ...process.env,
              ...shellEnv.env,
          ...
          const meta: string[] = []
          if (timedOut) meta.push(`bash tool terminated command after exceeding timeout ${timeout} ms`)
          ...
          // If process spawning isn't supported in this environment,
          // fall back to the host bridge via SSE.
        }
